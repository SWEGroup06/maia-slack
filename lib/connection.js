...
class Connection {
  ...
  tp(email, oldTitle, oldDateTime, newStartDateRange, newEndDateRange, newStartTimeRange, newEndTimeRange, newDayOfWeek) {
    return this._request(`/api/tp?organiserSlackEmail=${encodeURIComponent(JSON.stringify(email))}
                                    &oldTitle=${encodeURIComponent(JSON.stringify(oldTitle))}
                                    &oldDateTime=${encodeURIComponent(JSON.stringify(oldDateTime))}
                                    &newStartDateRange=${encodeURIComponent(JSON.stringify(newStartDateRange))}
                                    &newEndDateRange=${encodeURIComponent(JSON.stringify(newEndDateRange))}
                                    &newStartTimeRange=${encodeURIComponent(JSON.stringify(newStartTimeRange))}
                                    &newEndTimeRange=${encodeURIComponent(JSON.stringify(newEndTimeRange))}
                                    &newDayOfWeek=${encodeURIComponent(JSON.stringify(newDayOfWeek))}`)
  }
