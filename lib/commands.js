...
module.exports = function (CONFIG, web, CONN) {
  ...
  this.commands = {
    ...
    // setNotifyAutoReschedules: {
    //   desc: `Set whether you would like flexible events to be rescheduled automatically or sent as suggestions`,
    //   action: async function (data, msg) {
    //     try {
    //       const res = await CONN.setMinBreak(
    //           await context.aux.getEmail(msg.user), data.minBreakLength
    //       );
    //       if (context.aux.checkForResponseError(res, msg, 'setMinBreak')) {
    //         return;
    //       }
    //       if (res.success) {
    //         await web.chat.postMessage({
