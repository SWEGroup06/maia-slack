    ...
    schedule: {
      ...
      action: async function (data, msg) {
        try {
          ...
          if (!data.time) {
            // by default set start time to 2 hours from now at the *start* of that hour
            startOfSpecifiedDateRange = DateTime.local().plus(Duration.fromObject({hours: 2})).startOf('hour');
            endOfSpecifiedDateRange = startOfSpecifiedDateRange.plus({'day': 14}).startOf('hour');
          } else {
