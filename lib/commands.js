...
module.exports = function (CONFIG, web) {
  ...
  this.commands = {
    ...
    login: {
      ...
      action: function (msg) {
        context.conn.login(UTILS.getUID(msg.team, msg.user)).then(function (res) {
          if (!res || !res.url) {
            web.chat.postMessage({
              channel: msg.channel,
    ...
    meeting: {
      ...
      action: function (msg, match) {
        ...
        let userIds = [...ids].map(id => UTILS.getUID(msg.team, id));
        context.conn.meeting(userIds).then(function (res) {
          if (!res) {
            web.chat.postMessage({
              channel: msg.channel,
