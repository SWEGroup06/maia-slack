      ...
      action: async function (data, msg) {
        try {
          ...
          if (!data.time) {
            ...
            endOfSpecifiedDateRange = startOfSpecifiedDateRange.plus({'day': 14})
          } else {
          ...
          const res = await conn.schedule(title,
                                          emails,
          ...
          if (res.msg) {
            web.chat.postMessage({
              channel: msg.channel,
              text: UTILS.scheduleSlotNotFoundMessage()
            })
          } else {
            web.chat.postMessage({
              channel: msg.channel,
              text: UTILS.scheduleMessage(res.start, res.end, title)
            })
          }
        } catch (error) {
          context.aux.postError(msg, this.name, error);
    ...
    reschedule: {
      ...
      action: async function (data, msg) {
        try {
          ...
          const meetingTitle = data.meetingTitle === '' ? undefined : data.meetingTitle;
          ...
          const res = await conn.reschedule(email,
                                            oldStartDate,
                                            undefined,
                                            newStartDate.toISO(),
                                            newEndDate.toISO(), meetingTitle);
          if (context.aux.checkForResponseError(res, msg, this.name)) return;
          if (res.msg) {
            web.chat.postMessage({
              channel: msg.channel,
              text: `${UTILS.rescheduleSlotNotFoundMessage(meetingTitle)}`,
            })
          }
          web.chat.postMessage({
            channel: msg.channel,
            text: `${UTILS.rescheduleMessage(oldStartDate, res.start, meetingTitle)}`,
          })
        } catch (error) {
          context.aux.postError(msg, this.name, error);
        }
