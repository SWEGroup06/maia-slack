  ...
  this.commands = {
    ...
    login: {
      ...
      action: async function (msg) {
        try {
          const res = await context.conn.login(await getEmail(msg.user));
          if (!res) {
            web.chat.postMessage({
              channel: msg.channel,
    ...
    constraint: {
      private: true,
      /* */
      desc: `${CONFIG.prefix} constraint <startTime> <endTime> <dayOfWeek>`,
      regex: new RegExp(`${CONFIG.prefix} constraint (.+) (.+) (\\w+)`, 'i'),
      /**
       * regex[1] = startTime
       * regex[2] = endTime
       * regex[3] = dayOfWeek
      ...
      action: async function (msg, match) {
        try {
          const res = await context.conn.constraint(await getEmail(msg.user), match[1], match[2], match[3])
          if (!res) {
            web.chat.postMessage({
              channel: msg.channel,
              text: `> *MEETING ERROR: Invalid object response*`
          ...
          if (res.error) {
            web.chat.postMessage({
              channel: msg.channel,
              text: `> *CONSTRAINT ERROR: ${res.error}*`
            });
            return;
          }
          web.chat.postMessage({
