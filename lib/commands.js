...
const UI_CONSTRAINTS = require('./ui/constraints.js')
const UI_RESCHEDULE = require('./ui/reschedule.js')
  ...
  this.commands = {
    ...
    meeting: {
      ...
      action: async function (msg, match) {
        ...
        try {
          const res = await conn.meeting(await Promise.all([...userIDs].map(context.aux.getEmail)));
          if (!res) {
            web.chat.postMessage({
              channel: msg.channel,
    ...
    addConstraint: {
      ...
      action: async function (msg) {
        await web.chat.postMessage({
          channel: msg.channel,
          text: `> *Add constraint*\n> \`\`\`${Object.keys(context.commands).map(key => `- ${context.commands[key].desc}`).join("\n")}\`\`\``,
          blocks: UI_CONSTRAINTS.constraints,
    ...
    rescheduleWithUI: {
      ...
      action: async function (msg) {
        await web.chat.postMessage({
          channel: msg.channel,
          text: `> *Reschedule*\n> \`\`\`${Object.keys(context.commands).map(key => `- ${context.commands[key].desc}`).join("\n")}\`\`\``,
          blocks: UI_RESCHEDULE.reschedule,
          email: context.aux.getEmail(msg.user)
        });
      }
    },
