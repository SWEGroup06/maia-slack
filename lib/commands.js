    ...
    reschedule: {
      action: async function (data, msg) {
        try {
          ...
          const newStartDate = data.newMeetingDate[0];
          let newEndDate;
          ...
          if (data.newMeetingDate.length > 1) {
            ...
            newEndDate = data.newMeetingDate[1];
          } else {
            ...
            newEndDate = DateTime.fromISO(newStartDate).endOf('day').toISO();
          }

          const res = await conn.reschedule(email, oldStartDate, undefined, newStartDate, newEndDate);
