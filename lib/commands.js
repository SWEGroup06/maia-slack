...
module.exports = function (CONFIG, web, conn) {
  ...
  this.commands = {
      ...
      action: async function (data, msg) {
        try {
          ...
          if (!data.time) {
            ...
            startOfSpecifiedDateRange = DateTime.fromISO(data.time[0].split('+')[0]);
      ...
      action: async function (data, msg) {
        try {
          ...
          if (res.msg) {
            web.chat.postMessage({
              channel: msg.channel,
              text: `${UTILS.postScheduleErrorMessage(res.msg, meetingTitle)}`,
            })
          }
          web.chat.postMessage({
            channel: msg.channel,
            text: `${UTILS.postScheduleMessage(oldStartDate, res.start, meetingTitle)}`,
          })
        } catch (error) {
          context.aux.postError(msg, this.name, error);
        }
    ...
    // TODO: ?
    updateHistFreq: {
      desc: `Recalculate my historical preferences`,
      action: async function (data, msg) {
        try {
          const res = await conn.preferences(await context.aux.getEmail(msg.user));
          if (context.aux.checkForResponseError(res, msg, 'updateHistFreq')) return;
          if (res.success) {
            await web.chat.postMessage({
              channel: msg.channel,
    ...
    tp: {
      ...
      action: async function (data, msg) {
        try {
          ...
          /* TODO: ensure newstartdaterange is  */
          let newStartDateRange = data.newStartDateRange;
          if (!data.timeRangeSpecified) {
            newStartDateRange = DateTime.max(DateTime.fromISO(newStartDateRange),
                DateTime.local().plus({hours: 2}).startOf('hour')).toISO();
          }
          const res = await conn.tp(email, data.oldTitle, data.oldDateTime, newStartDateRange,
          ...
          if (res.msg) {
            web.chat.postMessage({
              channel: msg.channel,
              text: `${UTILS.rescheduleErrorMessage(res.msg, data.oldTitle)}`,
            })
          }
          web.chat.postMessage({
            channel: msg.channel,
            text: `${UTILS.rescheduleMessage(data.oldDateTime, res.start, data.oldTitle)}`,
          })
        } catch (error) {
          context.aux.postError(msg, this.name, error);
        }
