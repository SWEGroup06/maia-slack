...
module.exports = function(CONFIG, web, conn) {
  ...
  this.commands = {
    ...
    schedule: {
      action: async function (data, msg) {
        try {
          const res = await conn.schedule(await Promise.all(data.people.map((mention) => UTILS.getMentionId(mention.split(" ").join("").toUpperCase())).map(context.aux.getEmail)), data.startDate , data.endDate);
