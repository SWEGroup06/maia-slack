    ...
    schedule: {
      action: async function (data, msg) {
        ...
        try {
          ...
          // TODO: If date.time is length 1, then it's a specific time and specific date:
          //        - Set start of startOfSpecifiedDateRange to data.list[0]
          //        - Set start of endOfSpecifiedDateRange to ?????????
          // TODO: If length 2, then it means it's a range of times
          //        - Set start of startOfSpecifiedDateRange to data.list[0]
          //        - Set start of endOfSpecifiedDateRange to data.list[1]

          // Start datetime of range in which meeting should be scheduled to (as an ISO String).
          let startOfSpecifiedDateRange = DateTime.fromISO(data.time[0]);
          ...
          //   endOfSpecifiedDateRange = DateTime.fromISO(data.time[1]);
          // } else if (data.time.length === 1 && data.time[0] === '') {
          //   // No specific day or range has been specified!
          //   // Set a default new start and end date (we'll specify the possible range from today to two weeks later)
          //   startOfSpecifiedDateRange = DateTime.local();
          //   endOfSpecifiedDateRange = startOfSpecifiedDateRange.plus({days: 14});
          // } else {
          //   // This means schedule to a specific day! e.g. February 2nd, tomorrow, etc.
          //   // Create new date which is the end of the day of newStartDate
          //   if (startOfSpecifiedDateRange.startOf('day') <= DateTime.local().startOf('day')) {
          //     startOfSpecifiedDateRange = startOfSpecifiedDateRange.startOf('day');
          //   } else {
          //     // If the day is today, ensure the start of the specified range is not in the past!
          //     // i.e. startOfSpecifiedRange = the current moment in time
          //     startOfSpecifiedDateRange = DateTime.local();
          //   }
          //
          //   endOfSpecifiedDateRange = startOfSpecifiedDateRange.endOf('day');
          // }
          //
          // console.log("**************\n");
          // console.log(startOfSpecifiedDateRange.toISO());
          // console.log(endOfSpecifiedDateRange.toISO());
          // console.log("**************\n");
