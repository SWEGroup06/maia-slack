...
module.exports = function (CONFIG, web, conn) {
  ...
  this.commands = {
    ...
    tp: {
      ...
      action: async function (data, msg) {
        try {
          ...
          if (context.aux.checkForResponseError(res, msg, 'tp')) return;
          ...
          web.chat.postMessage({
            channel: msg.channel,
            text: `${UTILS.postScheduleMessage(data.oldDateTime, res.start, data.oldTitle)}`,
          })
        } catch (error) {
          context.aux.postError(msg, this.name, error);
        }
    ...
    // TODO: Temporarily as 'sp', which will soon be 'schedule'
    sp: {
      desc: `I am sp`,
      action: async function (data, msg) {
        try {
          ...
          const res = await conn.sp(slackAttendeesEmails, data.title, data.duration, data.startDateRange, data.endDateRange,
           data.startTimeRange, data.endTimeRange, data.flexible, data.dayOfWeek);
