      ...
      action: async function (data, msg) {
        try {
          ...
          if (!data.time) {
            ...
            startOfSpecifiedDateRange = DateTime.local().plus(
              Duration.fromObject({ hours: 2 })
            );
            endOfSpecifiedDateRange = startOfSpecifiedDateRange.plus({
              days: 14,
            });
          } else {
            startOfSpecifiedDateRange = DateTime.max(
              DateTime.fromISO(data.time[0].split("+")[0]),
              DateTime.local()
            );
            if (data.time.length > 1) {
              endOfSpecifiedDateRange = DateTime.fromISO(
                data.time[1].split("+")[0]
              );
            } else {
              endOfSpecifiedDateRange = startOfSpecifiedDateRange.plus({
          ...
          const res = await conn.schedule(
            title,
            emails,
            startOfSpecifiedDateRange.toISO(),
            endOfSpecifiedDateRange.toISO(),
            flexibility,
            duration,
            beforeAfterKey
          );
      ...
      action: async function (data, msg) {
        try {
          ...
          if (data.newMeetingDate.length > 1) {
            ...
            data.newMeetingDate.length === 1 &&
            data.newMeetingDate[0] === ""
          ) {
          ...
          if (res.msg) {
            web.chat.postMessage({
              channel: msg.channel,
              text: `${UTILS.repostScheduleErrorMessage(
                res.msg,
                meetingTitle
              )}`,
            });
          ...
          web.chat.postMessage({
            channel: msg.channel,
            text: `${UTILS.repostScheduleMessage(
              oldStartDate,
              res.start,
              meetingTitle
            )}`,
      ...
      action: async function (data, msg) {
        try {
          ...
          if (res.msg) {
            web.chat.postMessage({
              channel: msg.channel,
              text: `${UTILS.postScheduleErrorMessage(res.msg, data.oldTitle)}`,
            });
          }
          web.chat.postMessage({
            channel: msg.channel,
            text: `${UTILS.postScheduleMessage(
              data.oldDateTime,
              res.start,
              data.oldTitle
            )}`,
    ...
    sp: {
      ...
      action: async function (data, msg) {
        try {
          ...
          ids.unshift(msg.user); // Add organiser's id (user who sent message to maia)
          const slackAttendeesEmails = await Promise.all(ids.map(context.aux.getEmail))

          const res = await conn.sp(
          ...
          if (context.aux.checkForResponseError(res, msg, 'sp')) return;
          ...
          if (res.msg) {
            ...
            web.chat.postMessage({
              channel: msg.channel,
              text: `${UTILS.postScheduleMessage(res.start, res.end, data.title)}`,
            })
          }
        } catch (error) {
          context.aux.postError(msg, this.name, error);
