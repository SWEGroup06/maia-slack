...
module.exports = function (CONFIG, web, CONN) {
  ...
  this.commands = {
    ...
    constraint: {
      ...
      example: `maia modify my working hours`,
      action: async function (data, msg) {
        try {
          if (
            !data.busyTimes ||
    ...
    schedule: {
      ...
      example: `maia schedule a meeting for this week`,
      action: async function (data, msg) {
        try {
          const ids = [
            msg.user,
    ...
    reschedule: {
      ...
      example: `maia reschedule my meeting at 9am on Friday to February`,
      action: async function (data, msg) {
        try {
    ...
    cancel: {
      ...
      example: `maia cancel my 1pm on Friday`,
      action: async function (data, msg) {
        try {
          // Organiser email
          const email = await context.aux.getEmail(msg.user);
    ...
    recalibrate: {
      desc: `Recalibrate your preferences`,
      example: `maia recalibrate`,
      action: async function (data, msg) {
        try {
          ...
          if (res.success) {
            await web.chat.postMessage({
              channel: msg.channel,
              text: `Your historical data has been recalculated! :ok_hand:`,
            });
          }
        } catch (error) {
          context.aux.postError(msg, this.name, error);
    ...
    setMinBreak: {
      ...
      example: `maia set my break to 15 minutes`,
      action: async function (data, msg) {
        try {
