...
const messages = ['Sure thing!', 'No problem!', 'Of course!', 'Done!', 'Certainly!', 'No problem!'];

const emojis = [':blush:', ':thumbsup::skin-tone-4:', ':white_check_mark:', ':grin:', ':innocent:', ':smile:',
...
module.exports = {
  ...
  parseDate(date, month, time, ampm) {
    ...
    const d = new Date(`${date} ${month} ${new Date().getFullYear()} ${timeSplit[0]}:${timeSplit[1]}:00 GMT`);
  ...
  postScheduleErrorMessage(msg) {
    if (msg === 2) {
      const options = [`Uh oh it looks like there’s no time slot available within your constraints, please try another day`,
  ...
  rescheduleErrorMessage(msg, title) {
    if (msg === 2) {
      const options = [`Uh oh it looks like there’s no time slot available to reschedule ${title.length === 0 ? 'that event' : title} to within your constraints`,
      ];
      const x = Math.floor(Math.random() * options.length);
      return options[x];
    } else if (msg === 1) {
  ...
  postCancelMessage(title, dateTimeISO) {
    ...
    return `${messages[Math.floor(Math.random() * messages.length)]} ` +
     `${emojis[Math.floor(Math.random() * emojis.length)]} ` + `I've cancelled \"${title}\", which was booked for ` +
     `${dateTime.toLocaleString(DateTime.DATETIME_MED_WITH_WEEKDAY)}. :spiral_calendar_pad:`;
  },

  /**
   * TODO: Make this actually look like an error
   * @param msg
   * @return {*}
   */
