...
module.exports = {
  ...
  rescheduleMessage(oldDate, newDate, meetingTitle) {
    ...
    if (!oldDate) {
      return "Sure thing! :blush: I've moved " + (meetingTitle.length === 0 ? "your event" : meetingTitle) + " to `" + newDate + "`. :spiral_calendar_pad:"
    } else {
  ...
  scheduleMessage(start, end, title) {
    ...
    return `I've booked ${title.length === 0 ? 'an event' : title} from \*${start}\* to \*${end}\* :spiral_calendar_pad:. To reschedule, use \`maia reschedule\``
  },
  ...
  rescheduleSlotNotFoundMessage(title) {
